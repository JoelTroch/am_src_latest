...
void CCycler :: Spawn( )
{
	...
	if ( FClassnameIs( pev, "item_secret" ) )
	{
...
class CItemSecret : public CCycler
{
public:
	void Spawn();
	void Precache();
...
LINK_ENTITY_TO_CLASS( item_secret, CItemSecret );

void CItemSecret::Spawn()
{
	GenericCyclerSpawn( "models/secret.mdl", Vector( -16, -16, -16 ), Vector( 16, 16, 16 ) );
}

void CItemSecret::Precache()
{
...
void CItemSecret::GenericCyclerSpawn( char *szModel, Vector vecMin, Vector vecMax )
{
	if ( !szModel || !*szModel )
	{
		ALERT( at_console, "item_secret at %.0f %.0f %.0f has missing modelname\n", pev->origin.x, pev->origin.y, pev->origin.z );
		REMOVE_ENTITY( ENT( pev ) );
		return;
	}
	...
	pev->classname = MAKE_STRING( "item_secret" );
	PRECACHE_MODEL( szModel );
	SET_MODEL( ENT( pev ), szModel );
	CCycler::Spawn();
	UTIL_SetSize( pev, vecMin, vecMax );
