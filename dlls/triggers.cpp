...
LINK_ENTITY_TO_CLASS(npc_camera, CTriggerCamera);
...
void CTriggerCamera::Spawn( void )
{
	...
	if (FClassnameIs(pev, "npc_camera"))
	{
		m_flWait = 0.8f;
	}
}
...
void CTriggerCamera::FollowTarget( )
{
	...
	if (FClassnameIs(pev, "npc_camera"))
	{
		pev->avelocity.x = dx * gpGlobals->frametime*2000;
		pev->avelocity.y = dy * gpGlobals->frametime*2000;

	//	pev->velocity = pev->velocity * 33.8;
	//	pev->avelocity.x = dx * 2;
	//	pev->avelocity.y = dy * 2;
	 //	pev->avelocity = Vector(0, 0, 0);
...
// Shepard : Why it was named CFuncWhatever ? ^^
class CFuncSemtexZone : public CBaseEntity
{
public:
	void Spawn( void );
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
};
LINK_ENTITY_TO_CLASS( func_semtex_zone, CFuncSemtexZone );

void CFuncSemtexZone::Spawn( void )
{
	SET_MODEL( ENT( pev ), STRING( pev->model ) );
	pev->solid = SOLID_NOT;
	pev->movetype = MOVETYPE_NONE;
...
void CFuncSemtexZone::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( useType == USE_ON )
		pev->speed = 1.0;
		...
		pev->speed = (pev->speed == 1.0) ? 0.0 : 1.0;
}
...
void CTriggerCommand::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	{
		sprintf( szCommand, "%s\n", STRING( pev->netname ) );
		CLIENT_COMMAND( pActivator->edict(), szCommand );
	}
}
...
void CTriggerRandom::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	{
	case 0: FireTargets( STRING( m_iszTarget1 ), this, this, USE_ON, 0 ); break;
	case 1: FireTargets( STRING( m_iszTarget2 ), this, this, USE_ON, 0 ); break;
	case 2: FireTargets( STRING( m_iszTarget3 ), this, this, USE_ON, 0 ); break;
	case 3: FireTargets( STRING( m_iszTarget4 ), this, this, USE_ON, 0 ); break;
	}
}

class CFuncWeaponTailZone : public CBaseEntity
{
public:
	void Spawn( void );
};
LINK_ENTITY_TO_CLASS( func_weapon_tail_zone, CFuncWeaponTailZone );

void CFuncWeaponTailZone::Spawn( void )
{
	SET_MODEL( ENT( pev ), STRING( pev->model ) );
	pev->solid = SOLID_NOT;
	pev->movetype = MOVETYPE_NONE;
