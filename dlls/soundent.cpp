...
// sound info.
//=========================================================
void CSoundEnt::InsertSound(int iType, int iClass, const Vector &vecOrigin, int iVolume, float flDuration)
{
	int	iThisSound;
	...
	pSoundEnt->m_SoundPool[iThisSound].m_vecOrigin = vecOrigin;
	pSoundEnt->m_SoundPool[iThisSound].m_iType = iType;
	pSoundEnt->m_SoundPool[iThisSound].m_iClass = iClass;
	pSoundEnt->m_SoundPool[iThisSound].m_iVolume = iVolume;
	pSoundEnt->m_SoundPool[iThisSound].m_flExpireTime = gpGlobals->time + flDuration;
}
...
void CRadio::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if (!pCaller){
		ALERT(at_console, "No caller and activator for radio: ");
	...
	if ( !FStringNull( pev->message ) && strlen( szSoundFile ) > 1 )
	{
		EMIT_SOUND_FMOD_2D_ONE(pPlayer->edict(), szSoundFile, 1.0f, 1);
	//	EMIT_SOUND_FMOD_2D_ALL(szSoundFile);
	}

	char* szTitle = (char*)STRING( pev->netname );
	if ( !FStringNull( pev->netname ) && strlen( szTitle ) > 1 )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgRadioIcon, NULL, pActivator->pev );
