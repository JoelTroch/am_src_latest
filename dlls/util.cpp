...
void UTIL_EmitAmbientSound(edict_t *entity, const Vector &vecOrigin, const char *samp, float vol, float attenuation, int fFlags, int pitch)
{
	...
	if (samp && *samp == '!')
	{
		...
		UTIL_ShowMessagePVS(samp, vecOrigin); // buz
	}
	else
...
{
	if (msg_dest == HUD_PRINTCENTER)// Sys: don't use those crappy and ugly fonts!
	{
		CBaseEntity *pPlayer = UTIL_PlayerByIndex(1);

		if (pPlayer)
			UTIL_ShowMessage(msg_name, pPlayer);
	}
	else
	{
...
void UTIL_BloodDrips( const Vector &origin, const Vector &direction, int color, int amount )
{
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgParticles );
		...
		WRITE_COORD( 0 );
		WRITE_SHORT( 0 );
		WRITE_STRING( "engine_blood_impact.txt" );
	MESSAGE_END();
}
...
void UTIL_CustomDecal(TraceResult *pTrace, const char *name, int persistent)
{
	...
	if (pTrace->pHit && VARS(pTrace->pHit)->rendermode == kRenderTransAlpha)
		return;

	if (pTrace->pHit)
		idx = ENTINDEX(pTrace->pHit);
...
void UTIL_DecalTrace( TraceResult *pTrace, int decalNumber )
{
	...
	case DECAL_GUNSHOT4:
	case DECAL_GUNSHOT5:
		UTIL_CustomDecal(pTrace, "wallshot", 0);
		break;

	case DECAL_GLASSBREAK1:
	case DECAL_GLASSBREAK2:
	case DECAL_GLASSBREAK3:
		UTIL_CustomDecal(pTrace, "glassbreak", 0);
		break;

	case DECAL_SCORCH1:
	case DECAL_SCORCH2:
		UTIL_CustomDecal(pTrace, "scorch", 0);
		break;
	...
	case DECAL_BLOOD4:
	case DECAL_BLOOD5:
		UTIL_CustomDecal(pTrace, "redblood", 0);
		break;
	}
