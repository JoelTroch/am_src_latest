...
void CBaseMonster :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	else if ( pev->flags & FL_MONSTER )
	{
		...
		SetTouch( NULL );
	//	SetTouch(&CBaseMonster::BounceCorpse);
...
/*	if (pev->rendermode == kRenderNormal)
	...
	//SYS: new engine draws transparent textures in a weird way, set this later
...
void CBaseEntity :: SUB_FadeOut ( void  )
{
	...
	//	ALERT( at_console, "SUB_Remove\n" );
		pev->renderamt = 0;
		pev->nextthink = gpGlobals->time + 1;//0.2
...
void CBaseEntity::FireBullets(ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker )
{
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			{
				float xx, yy, zz;
				xx = tr.vecEndPos.x - tr.vecPlaneNormal.x;
				yy = tr.vecEndPos.y - tr.vecPlaneNormal.y;
				zz = tr.vecEndPos.z - tr.vecPlaneNormal.z;

				char texType = TEXTURETYPE_FindTrace(&tr, vecSrc, vecEnd);
				MESSAGE_BEGIN(MSG_ALL, gmsgSpecMsg);
				WRITE_BYTE(CL_SHOTPARTICLE);
				WRITE_BYTE(iBulletType);
				...
				UTIL_DecalTrace(&tr, DECAL_GUNSHOT1 + RANDOM_LONG(0, 5));
			}
...
float CBaseEntity::GetBulletDamage(int type)
{
	switch (type)
	{
	case BULLET_PLAYER_1911:
		return gSkillData.plrDmg1911;
	case BULLET_PLAYER_AK47:
		return gSkillData.plrDmgAK47;
	case BULLET_PLAYER_ANACONDA:
		return gSkillData.plrDmgAnaconda;
	case BULLET_PLAYER_BERETTA:
		return gSkillData.plrDmgBeretta;
	case BULLET_PLAYER_BIZON:
		return gSkillData.plrDmgBizon;
	case BULLET_PLAYER_DEAGLE:
		return gSkillData.plrDmgDeagle;
	case BULLET_PLAYER_FAMAS:
		return gSkillData.plrDmgFAMAS;
	case BULLET_PLAYER_GLOCK:
		return gSkillData.plrDmgGlock;
	case BULLET_PLAYER_L96A1:
		return gSkillData.plrDmgL96A1;
	case BULLET_PLAYER_LESSLETHAL:
		return gSkillData.plrDmgLessLethal;
	case BULLET_PLAYER_M14:
		return gSkillData.plrDmgM14;
	case BULLET_PLAYER_M249:
		return gSkillData.plrDmgM249;
	case BULLET_PLAYER_M4A1:
		return gSkillData.plrDmgM4A1;
	case BULLET_PLAYER_MP5:
		return gSkillData.plrDmgMP5;
	case BULLET_PLAYER_P90:
		return gSkillData.plrDmgP90;
	case BULLET_PLAYER_SPAS12:
		return gSkillData.plrDmgSPAS12;
	case BULLET_PLAYER_USAS:
		return gSkillData.plrDmgUSAS;
	}
	return 50;
}
...
{
/*	DynamicPunch(SPREAD_DEFAULT);//moved up for test

	CBasePlayer *pPlayer = GetClassPtr((CBasePlayer *)pev);

	if ((pPlayer->pev->weapons & (1 << WEAPON_SUIT)) && (pPlayer->m_pActiveItem->m_iId != NULL))//FIX
	{
		switch (pPlayer->m_pActiveItem->m_iId)
		{
		case  WEAPON_SPAS12:					vecSpread = VECTOR_CONE_10DEGREES; break;
		case  WEAPON_USAS:						vecSpread = VECTOR_CONE_10DEGREES; break;
		case  WEAPON_LESSLETHAL:		vecSpread = VECTOR_CONE_10DEGREES; break;
		case  WEAPON_L96A1:

			if (pPlayer->pev->velocity.Length2D() > 5)//running
				vecSpread = VECTOR_CONE_15DEGREES;
			else
				vecSpread = VECTOR_CONE_PERFECT;

			break;

		default:

			if (pPlayer->pev->velocity.Length2D() < 120)//walk
			{
				if (pPlayer->pev->flags & FL_ONGROUND)
				{
					if (pPlayer->m_pActiveItem->m_iId == WEAPON_M14 || pPlayer->m_pActiveItem->m_iId == WEAPON_L96A1)
						vecSpread = VECTOR_CONE_PERFECT;
					else
					{
	...
	*/
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		{
			CBaseEntity *pEntity = CBaseEntity::Instance(tr.pHit);
			pEntity->TraceAttack( pevAttacker, GetBulletDamage(iBulletType), vecDir, &tr, (iBulletType == BULLET_PLAYER_LESSLETHAL) ? DMG_PELLETS : DMG_BULLET );
			...
			if (pEntity->IsBSPModel())
			{
				// Explosive rounds for the USAS-12
				if (iBulletType == BULLET_PLAYER_USAS)
				{
					...
					WRITE_BYTE(3); // scale
					WRITE_BYTE(33); // framerate
					WRITE_BYTE(TE_EXPLFLAG_NONE);
					MESSAGE_END();

					::RadiusDamage(tr.vecEndPos, pev, pev, 20, 128, CLASS_NONE, DMG_BLAST);
					UTIL_DecalTrace(&tr, DECAL_SMALLSCORCH1 + RANDOM_LONG(0, 2));
				}

				//go through walls
				//SYS - BUG: this works okay, but a concrete particle will appear when monster is hit
			/*	UTIL_TraceLine(tr.vecEndPos + vecDir * 10, vecSrc, dont_ignore_monsters, ENT(pev), &tr);
				if (!tr.fAllSolid)
				{
					UTIL_DecalTrace(&tr, DECAL_GUNSHOT1 + RANDOM_LONG(0, 5));
					DecalGunshot(&tr, iBulletType);
					...
					WRITE_COORD(0);
					WRITE_SHORT(0);
					WRITE_STRING("engine_concrete_impact.txt");
					MESSAGE_END();
				}*/
			}
		}
...
void CBaseEntity::FireBulletsWater(Vector vecSrc, Vector vecEnd)
{
	...
	/*	MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
		...
		WRITE_COORD(0);
		WRITE_SHORT(0);
		WRITE_STRING("engine_water_impact.txt");
		MESSAGE_END();*/
	...
	//	ALERT(at_console, "hitting water\n" );  
	}
	else
	{
...
//	DynamicPunch
//=======================
// Creates dynamic punch angle. 
// This code checks what kind of weapon the player is using and creates a specific punch angle for it
// This func is called in each FireBulletsPlayer and FireBulletsThroughWalls
//
// Yes, I'm so lazy. I could call this func in each weapon... but... well...
// Actually, iWeaponType does nothing. I hold it here. Maybe it's called from another file.
// If so iWeaponType is used, instead of fMySpread;

 
void CBaseEntity::DynamicPunch(int iWeaponType)
{
	CBasePlayer *m_pPlayer = GetClassPtr((CBasePlayer *)pev);
	CBasePlayerWeapon *gun = (CBasePlayerWeapon *)m_pPlayer->m_pActiveItem->GetWeaponPtr();

	Vector vecDir;
	float fSpreadP, fSpreadY, fSpreadR, fMySpread;

	if ((m_pPlayer->pev->weapons & (1 << WEAPON_SUIT)) && (m_pPlayer->m_pActiveItem->m_iId != NULL))//FIX
	{
		switch (m_pPlayer->m_pActiveItem->m_iId)
		{
		case  WEAPON_1911:		fMySpread = 0.8; break;//1911
		case  WEAPON_ANACONDA:	fMySpread = 4.0; break;
		case  WEAPON_MP5:		fMySpread = 0.8; break;
		case  WEAPON_SPAS12:	fMySpread = 4.0; break;//3.5
		case  WEAPON_USAS:		fMySpread = 3.0; break;
		case  WEAPON_AK47:		fMySpread = 1.6; break;
		case  WEAPON_M4A1:		fMySpread = 1.0; break;
		case  WEAPON_DEAGLE:	fMySpread = 2.0; break;
		case  WEAPON_M14:	fMySpread = 3.0; break;
		case  WEAPON_FAMAS:		fMySpread = 0.8; break;
		case  WEAPON_M249:		fMySpread = 1.8; break;
		case  WEAPON_P90:		fMySpread = 1.0; break;//p90
		case  WEAPON_BIZON:		fMySpread = 1.5; break;
		case  WEAPON_BERETTA:	fMySpread = 0.8; break;
		case  WEAPON_L96A1:		fMySpread = 1.0; break;
		case  WEAPON_GLOCK:	fMySpread = 0.8; break;
		case  WEAPON_LESSLETHAL:		fMySpread = 2.0; break;

		default:				fMySpread = 50.0; break;//ugly way to notice a weapon isn't using spread
		}
	}
	else// fix possible bug
	{
		fMySpread = 0.0;
		fSpreadP = 0.0;
		fSpreadY = 0.0;
	...
	if (m_pPlayer->pev->velocity.Length2D() <= 120)//walk
	{
	...
	else//running!
	{
		...
		{
			fSpreadP = 2.6;//2.0
			fSpreadY = 2.4;//0.0 2.0 ??
			fSpreadR = 2.4;
		}
		else // si no esta en el suelo es porque esta en el aire
		{
			fSpreadP = 5.0;
			fSpreadY = 5.0;//0.0 3.0
			fSpreadR = 5.0;
		}
	}

	if (iWeaponType != SPREAD_DEFAULT)
	{
		fSpreadP += iWeaponType;
		fSpreadY += iWeaponType;
		fSpreadR += iWeaponType;
	}
	else//false, let's use normal spread (the original one that comes with each weapon)
	{
		fSpreadP += fMySpread;
		fSpreadY += fMySpread;
		fSpreadR += fMySpread;
...
//	UTIL_ScreenShakeOne(m_pPlayer, 2, fMySpread, 0.3);
	...
	if (gun)
	{
		...
		//dump, finnally, punch angle
	/*	if (m_pPlayer->m_bIron || gun->m_fDefaultAnim == FIREMODE_SEMI)
		{
			fSpreadP = fSpreadP / 2;
			fSpreadY = fSpreadY / 2;
			fSpreadR = fSpreadR / 2;
		...
		/*   	MESSAGE_BEGIN( MSG_ONE, gmsgRecoil, NULL, pev );
		WRITE_BYTE(fSpreadP);
		MESSAGE_END();*/


	}
//	m_pPlayer->pev->punchangle.x += fSpreadP;
//	m_pPlayer->pev->punchangle.y += RANDOM_FLOAT(-fSpreadY, fSpreadY);
//	m_pPlayer->pev->punchangle.z += RANDOM_FLOAT(-fSpreadR, fSpreadR);


	m_pPlayer->ViewPunch(RANDOM_FLOAT(0, fSpreadP), RANDOM_FLOAT(-fSpreadY, fSpreadY), RANDOM_FLOAT(-fSpreadR, fSpreadR));
//	m_pPlayer->ViewPunch(1, 0, 0);
