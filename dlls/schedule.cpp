...
//				return;
	...
	case TASK_FIND_FAR_NODE_COVER_FROM_ORIGIN:
		{
			if ( FindCover( pev->origin, pev->view_ofs, pTask->flData, CoverRadius() * 2 ) )
			{
				TaskComplete();	// Try for cover further away than task data
			}
			else if ( FindCover( pev->origin, pev->view_ofs, 0, pTask->flData ) )
			{
				TaskComplete(); // Otherwise try for any cover
			}
			else
			{
				// no cover!
	...
	case TASK_GET_PATH_TO_SPOT:
		...
		case TASK_GET_PATH_TO_ITEM:
		{
			ALERT(at_aiconsole, "TASK_GET_PATH_TO_ITEM\n");

			CBaseEntity *pItem = CBaseEntity::Instance(FIND_ENTITY_BY_CLASSNAME(NULL, "weapon_mp5"));

			if (pItem && MoveToLocation(m_movementActivity, 2, pItem->Center()))
			{
				TaskComplete();
			}
			else
		...
		case TASK_GET_PATH_TO_AI_HOTSPOT:
		{
			ALERT(at_aiconsole, "TASK_GET_PATH_TO_AI_HOTSPOT\n");

			CBaseEntity *pMarker = CBaseEntity::Instance(FIND_ENTITY_BY_CLASSNAME(NULL, "ai_hotspot"));

			if (pMarker && MoveToLocation(m_movementActivity, 2, pMarker->Center()))
			{
				TaskComplete();
			}
			else
