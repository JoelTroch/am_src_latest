...
#define SF_BREAK_MELEE			256// instant break if hit with melee
#define SF_BREAK_SEMTEX_ONLY	1024// only the semtex can break it
...
int SENTENCEG_PlayRndSz(edict_t *entity, const char *szrootname, float volume, float attenuation, int flags, int pitch, bool bRadio);
...
void EMIT_SOUND_FMOD_2D_ONE(edict_t *tEntity, const char *cSample, float flPitch = 1.0f, int iLookUpSubtitles = 0 );
void EMIT_SOUND_FMOD_2D_ALL( const char *cSample, float flPitch = 1.0f );
void EMIT_SOUND_FMOD_3D( edict_t *tEntity, const char *cSample, float flMinDistance = 512.0f, float flMaxDistance = -1.0f, int iEntityIndexToFollow = -1, float flPitch = 1.0f );
