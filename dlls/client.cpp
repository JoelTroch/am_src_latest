...
void respawn(entvars_t* pev, BOOL fCopyCorpse)
{
	...
	// Shepard : Now handled by my easter egg ^^
	else
...
void ClientCommand( edict_t *pEntity )
{
	...
	else if (FStrEq(pcmd, "installmod"))
	{
		CBasePlayerItem *item = GetClassPtr((CBasePlayer *)pev)->m_pActiveItem;
		if (item && (strstr(STRING(item->pev->classname), "weapon_") != NULL))
		{
			CBasePlayerWeapon *wpn = (CBasePlayerWeapon*)item;
			if (wpn && wpn->IsAmWeapon())
			{
				CBaseAMWeapon *amw = (CBaseAMWeapon*)wpn;
				if (amw)
					amw->InstallMod(atoi(CMD_ARGV(1)));
			}
		}

	}
	else if (FStrEq(pcmd, "mod_silencer"))	{ SERVER_COMMAND("installmod 4\n");		}
	else if (FStrEq(pcmd, "mod_firemode"))	{ SERVER_COMMAND("installmod 32\n");	}
	else if (FStrEq(pcmd, "mod_scope"))		{ SERVER_COMMAND("installmod 8\n");		}
	else if (FStrEq(pcmd, "mod_gl"))		
	{
		SERVER_COMMAND("ak_test 1\n");		
		EMIT_SOUND_FMOD_3D(ENT(pev), "items/spray.ogg");
	}
...
#define MOD_OFF				2
	...
	*/
		...
		if (pl)
		{
			if (pl->m_pActiveItem)
			{
				...
				if (gun)
				{
					...
					WRITE_BYTE(MOD_OFF);//iExtendedMag
					WRITE_BYTE(MOD_OFF);//iGL
	...
	else if (FStrEq(pcmd, "epipen"))
	{
		GetClassPtr((CBasePlayer *)pev)->EpipenPrepare();
	}
	else if (FStrEq(pcmd, "grenade"))
	{
		GetClassPtr((CBasePlayer *)pev)->GrenadePrepare();
	}
	else if (FStrEq(pcmd, "loot"))
	{
		...
		for (int i = 0; i <= 100; i++)
		{
			...
			BeastType = 0; //Since we have 6 beasts in our array
...
void ClientPrecache( void )
{
	...
	PRECACHE_SOUND("player/jump01.wav");		// jump
	PRECACHE_SOUND("player/jump02.wav");		// jump
	PRECACHE_SOUND("player/jump03.wav");		// jump
	PRECACHE_SOUND("player/jump04.wav");		// jump
...
// player pain sounds
	...
	PRECACHE_MODEL("models/p_epipen.mdl");
	PRECACHE_MODEL("models/v_epipen.mdl");
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					if ( II.iId >= 0 && II.iId < 32 )
					{
						...
						item->iuser3					= gun->m_fireState; // Shepard : Was used by HL's Egon, can be either removed or used for something else
...
//	CBasePlayer *pl = (CBasePlayer *)CBasePlayer::Instance(pev);
	...
	if ( sendweapons )
	{
		if ( pl )
		{
			...
			cd->vuser1.y		= pl->ammo_heal;
			cd->vuser1.z		= pl->ammo_50;
			cd->ammo_nails		= pl->ammo_45;
			cd->ammo_shells		= pl->ammo_buckshot;
			cd->ammo_rockets	= pl->ammo_rockets;
			cd->ammo_cells		= pl->ammo_44;
			cd->vuser2.x		= pl->ammo_57;
