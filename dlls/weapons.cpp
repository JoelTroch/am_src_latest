...
void LoadWeaponSettings(char *filename); // buz
void DebugWeaponSettings(void); // buz
void LoadNames(void);
...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "item_epipen" );
	UTIL_PrecacheOther( "item_grenade" );
	...
	// Flashlight
	UTIL_PrecacheOtherWeapon( "weapon_flash" );

	// Fists
	UTIL_PrecacheOtherWeapon( "weapon_fists" );

	// PDA
	UTIL_PrecacheOtherWeapon( "weapon_pda" );

	// Semtex charge
	UTIL_PrecacheOtherWeapon( "weapon_semtex" );

	// Rose
	UTIL_PrecacheOtherWeapon( "weapon_rose" );
	...
	UTIL_PrecacheOther( "ammo_deagleclip" );

	// Colt M1911A2 45 ACP
	UTIL_PrecacheOtherWeapon( "weapon_1911" );
	UTIL_PrecacheOther( "ammo_1911clip" );

	// Beretta 92 FS 9mm
	UTIL_PrecacheOtherWeapon( "weapon_ber92f" );
	UTIL_PrecacheOther( "ammo_ber92fclip" );

	// Glock 18 9mm
	UTIL_PrecacheOtherWeapon( "weapon_glock18" );
	UTIL_PrecacheOther( "ammo_glock18clip" );

	// Colt Anaconda (Python) .44
	UTIL_PrecacheOtherWeapon( "weapon_anaconda" );
	UTIL_PrecacheOther( "ammo_44" );
	...
	UTIL_PrecacheOther( "ammo_p90clip" );
	...
	UTIL_PrecacheOther( "ammo_famasclip" );

	// Colt M4A1 5.56
	UTIL_PrecacheOtherWeapon( "weapon_m4a1" );
	UTIL_PrecacheOther( "ammo_m4a1clip" );
	...
	UTIL_PrecacheOther( "ammo_m249clip" );
	...
	// SPAS 12
	UTIL_PrecacheOtherWeapon( "weapon_spas12" );
	UTIL_PrecacheOther( "ammo_buckshot" );

	// Less Lethal shotgun
	UTIL_PrecacheOtherWeapon( "weapon_shotgun_non_lethal" );
	...
	// M14 7.62
	UTIL_PrecacheOtherWeapon( "weapon_m14" );
	UTIL_PrecacheOther( "ammo_m14clip" );

	// L96A1 7.62
	UTIL_PrecacheOtherWeapon( "weapon_l96a1" );
	UTIL_PrecacheOther( "ammo_l96a1clip" );
	...
	LoadWeaponSettings("scripts/weapon_settings.ini");
	DebugWeaponSettings();
	LoadNames();
}
...
void CBasePlayerItem::FallThink ( void )
{
	...
	if ( pev->flags & FL_ONGROUND )
	{
		...
		if ( !FNullEnt( pev->owner ) )
			EMIT_SOUND_FMOD_3D( ENT( pev ), "items/weapondrop1.wav", 512.0f, -1.0f, this->entindex() );
...
void CBasePlayerItem::Materialize( void )
{
	if ( pev->effects & EF_NODRAW )
	{
		...
		EMIT_SOUND_FMOD_3D( ENT( pev ), "items/suitchargeok1.wav", 512.0f, -1.0f, this->entindex() );
...
void CBasePlayerItem::DefaultTouch( CBaseEntity *pOther )
{
	...
	if (pOther->AddPlayerItem( this ))
	{
		...
		EMIT_SOUND_FMOD_3D( ENT( pPlayer->pev ), "items/gunpickup2.wav", 512.0f, -1.0f, m_pPlayer->entindex() );
	}
...
void CBasePlayerWeapon::ItemPreFrame()
{
	...
	if (II.iMaxClip != m_pPlayer->iClientMaxClip)
	{
		MESSAGE_BEGIN(MSG_ONE, gmsgSpecMsg, NULL, m_pPlayer->pev);
		WRITE_BYTE(CL_CLIPSIZE);
		WRITE_BYTE(II.iMaxClip);
		MESSAGE_END();

		m_pPlayer->iClientMaxClip = II.iMaxClip;
		ALERT(at_console, "update clip data %i\n", II.iMaxClip);
	}
}
...
int CBasePlayerWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
	BOOL bSend = FALSE;
	int state = 0;
	CBaseAMWeapon *amWeaponPointer = NULL;
	if (IsAmWeapon())
		amWeaponPointer = (CBaseAMWeapon*)this;
	...
	if ( m_iClip != m_iClientClip ||
		 ...
		 pPlayer->m_iFOV != pPlayer->m_iClientFOV || ((amWeaponPointer && (amWeaponPointer->installedMods != amWeaponPointer->clientMods))))
	{
		bSend = TRUE;
	}
	...
	if ( bSend )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgCurWeapon, NULL, pPlayer->pev );
			...
			if (amWeaponPointer){
				WRITE_BYTE(amWeaponPointer->installedMods);
				WRITE_BYTE(amWeaponPointer->GetAvailableMods());
			}
			else{
		...
		if (amWeaponPointer)
			amWeaponPointer->clientMods = amWeaponPointer->installedMods;
	}
...
BOOL CBasePlayerWeapon :: AddPrimaryAmmo( int iCount, char *szName, int iMaxClip, int iMaxCarry )
{
	...
	if (iIdAmmo > 0)
	{
		...
		if (m_pPlayer->HasPlayerItem( this ) )
		{
			...
			EMIT_SOUND_FMOD_3D( ENT( pev ), "items/9mmclip1.wav", 512.0f, -1.0f, m_pPlayer->entindex() );
		}
	}
...
BOOL CBasePlayerWeapon :: PlayEmptySound( void )
{
	...
	{
		EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "weapons/357_cock1.wav", 512.0f, -1.0f, m_pPlayer->entindex());

		if (fSpeakTime<= gpGlobals->time)
		{
			switch (RANDOM_LONG(0, 5))
			{
			case 0: EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "fred/world/empty1_stealth.wav", 512.0f, -1.0f, m_pPlayer->entindex()); break;
			case 1: EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "fred/world/empty2_stealth.wav", 512.0f, -1.0f, m_pPlayer->entindex()); break;
			case 2: EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "fred/world/empty3_stealth.wav", 512.0f, -1.0f, m_pPlayer->entindex()); break;
			case 3: EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "fred/world/empty4_stealth.wav", 512.0f, -1.0f, m_pPlayer->entindex()); break;
			case 4: EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "fred/world/empty5_stealth.wav", 512.0f, -1.0f, m_pPlayer->entindex()); break;
			case 5: EMIT_SOUND_FMOD_3D(ENT(m_pPlayer->pev), "fred/world/empty6_stealth.wav", 512.0f, -1.0f, m_pPlayer->entindex()); break;
			}
			fSpeakTime = gpGlobals->time + 2;
		}
...
void CBasePlayerAmmo::Materialize( void )
{
	...
	{
		// changing from invisible state to visible.
		EMIT_SOUND_FMOD_3D( ENT( pev ), "items/gunpickup2.wav", 2048.0f );
		pev->effects &= ~EF_NODRAW;
		pev->effects |= EF_MUZZLEFLASH;
	}
...
float g_wep_origin_x = 0.0;
float g_wep_origin_y = 0.0;
float g_wep_origin_z = 0.0;
...
void CBasePlayerWeapon::UpdateItemInfo(void)
{
	...
	if (GetItemInfo(&iInfo))
	{
		//if (iInfo.weaponName)
		{
		//	CVAR_SET_FLOAT("clip_size", iInfo.iMaxClip);

			if (!m_pMySettings)
			{
				SetTableSettingsPointer();
			}

			g_wep_origin_x = m_pMySettings->origin_x;
			g_wep_origin_y = m_pMySettings->origin_y;
			g_wep_origin_z = m_pMySettings->origin_z;

		/*	ALERT(at_console, "|   \n");
			ALERT(at_console, "|    loaded origin_x: %f\n", g_wep_origin_x);
			ALERT(at_console, "|    loaded origin_y: %f\n", g_wep_origin_y);
			ALERT(at_console, "|    loaded origin_z: %f\n", g_wep_origin_z);*/

			//SORRY GUYS, DON'T TIME TO DO THIS, PLEASE SEND A CLIENT MESSAGE AND SAVE THE WORLD
			CVAR_SET_FLOAT("origin_x", g_wep_origin_x);
			CVAR_SET_FLOAT("origin_y", g_wep_origin_y);
			CVAR_SET_FLOAT("origin_z", g_wep_origin_z);
		}
	}
}
...
// to deploy a better weapon that the player may pick up because he has no ammo for it.
	...
	EMIT_SOUND_FMOD_3D( pOther->edict(), "items/gunpickup2.wav", 512.0f, -1.0f, pOther->entindex() );
	SetTouch(NULL);
	UTIL_Remove(this);
}
...
BOOL CWeaponBox::PackWeapon( CBasePlayerItem *pWeapon )
{
	...
	pWeapon->SetThink( NULL ); // melee may be trying to swing again, etc.
...
extern wep_params_t	gWeaponSettings[16];
extern wep_params_t	gDefaultSettings;

extern int				gSCounter;

void CBasePlayerWeapon::SetTableSettingsPointer(void)
{
	const char *name = STRING(pev->classname);
	for (int i = 0; i < gSCounter; i++)
	{
		if (!strcmp(name, gWeaponSettings[i].szWeaponName))
		{
			m_pMySettings = &gWeaponSettings[i];
			break;
		}
	}
	...
	if (!m_pMySettings)
	{
		...
		ALERT(at_console, " *\n");
		m_pMySettings = &gDefaultSettings;
	}
}
...
TYPEDESCRIPTION	CSemtex::m_SaveData[] =
{
	DEFINE_FIELD( CSemtex, m_chargeReady, FIELD_INTEGER )
};
IMPLEMENT_SAVERESTORE( CSemtex, CBaseAMWeapon );
