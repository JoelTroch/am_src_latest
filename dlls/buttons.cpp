...
public:
	...
	float m_flOffTime;
	float m_flStartTime;
	float m_flLastActTime;
	edict_t *m_pLastOwner;
	string_t m_sSecondTarget;
};
...
TYPEDESCRIPTION CFuncTButton::m_SaveData[] =
{
	...
	DEFINE_FIELD( CFuncTButton, m_bInUse, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFuncTButton, m_flOnTime, FIELD_FLOAT ),
	DEFINE_FIELD( CFuncTButton, m_flOffTime, FIELD_FLOAT ),
	DEFINE_FIELD( CFuncTButton, m_flStartTime, FIELD_TIME ),
	DEFINE_FIELD( CFuncTButton, m_flLastActTime, FIELD_TIME ),
	DEFINE_FIELD( CFuncTButton, m_sSecondTarget, FIELD_STRING ),
};
IMPLEMENT_SAVERESTORE( CFuncTButton, CBaseEntity );

void CFuncTButton::Spawn()
{
	pev->movetype = MOVETYPE_PUSHSTEP;
	// Must use SOLID_BBOX instead of SOLID_BSP.
	// SOLID_BSP implies MOVETYPE_PUSH which calls Think when the object moves. The original devs made a huge HACK to support decals.
...
void CFuncTButton::KeyValue( KeyValueData *pkvd )
{
	...
	{
		m_flOnTime = atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "off_time" ) )
	{
		m_flOffTime = atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "starget" ) )
	{
		m_sSecondTarget = ALLOC_STRING( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
		CBaseToggle::KeyValue( pkvd );
...
void CFuncTButton::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( ( ( m_flLastActTime + 2.0 ) > gpGlobals->time ) && ( pActivator->edict() == m_pLastOwner ) ) // Not so fast
		return;
	...
	if ( !m_bState )
	{
		if ( ( m_flStartTime + m_flOnTime ) <= gpGlobals->time ) // Check if it has to be enabled
		{
			...
			pev->frame = 1; // Switch texture
			SUB_UseTargets( CBaseEntity::Instance( pev->owner ), USE_TOGGLE, 0 ); // Fire target
			UTIL_ShowMessage( STRING( pev->message ), pActivator );
			m_flLastActTime = gpGlobals->time;
			m_pLastOwner = pev->owner;
	...
	else // Trying to disable it
	{
		if ( ( m_flStartTime + m_flOffTime ) <= gpGlobals->time ) // Check if it has to be disabled
		{
			...
			pev->frame = 0; // Switch texture
			if ( !FStringNull( m_sSecondTarget ) )
				FireTargets( STRING( m_sSecondTarget ), CBaseEntity::Instance( pev->owner ), this, USE_TOGGLE, 0 ); // Fire target
...
void CFuncTButton::BThink()
{
	Vector vecLOS;
	float flMaxDot = VIEW_FIELD_NARROW;
	float flDot;
	float flDist;
	// Check if player hasn't released the button
	if ( pev->owner && CBaseEntity::Instance( pev->owner ) )
	{
		// Check if player has the button in it's line of sight
		...
		if ( !( VARS( pev->owner )->button & IN_USE ) || ( flDot <= flMaxDot ) || ( flDist > 64 ) )
		{
			// Button's no longer used
			m_bInUse = false;
			m_flStartTime = 0.0;
	...
	float flPercentage = ( gpGlobals->time - m_flStartTime ) / m_flOnTime;
	
//	ALERT(at_console, "pourcentage: %f\n", flPercentage);
	if (m_bInUse)
	{
		char szText[201];
		hudtextparms_t     hText;
		sprintf(szText, "Decoding: %i", int(flPercentage * 100));
