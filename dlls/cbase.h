...
public:
	...
	float GetBulletDamage(int type);

	// path corners
	CBaseEntity			*m_pGoalEnt;// path corner we are heading towards
	CBaseEntity			*m_pLink;// used for temporary link-list operations.
	int					b_PlayerIsNearOf;
	BOOL				b_FlashedMonster;
...
// still realize that they are teammates. (overridden for monsters that form groups)
	...
	virtual CBaseHuman *MyHumanPointer(void) { return NULL; }
...
//	virtual void	SetActivator( CBaseEntity *pActivator ) {}
	...
	void		DynamicPunch(int iWeaponType);
	...
	int ammo_556;
	int ammo_762;
	//Special stuff for grenades and semtex.
	float m_flStartThrow;
	float m_flReleaseThrow;
	int m_chargeReady;
	int m_fInAttack;
	int m_fireState; // Shepard : Was used by HL's Egon, can be either removed or used for something else
};
...
#define DMG_PELLETS			(1 << 30)	// shotgun blast
...
class CGameMusic : public CPointEntity
{
public:
	void Spawn();
	void KeyValue( KeyValueData* pkvd );
	...
	static TYPEDESCRIPTION m_SaveData[];
	string_t m_szMusicAction;
	string_t m_szMusicStealth;
};
