...
CTeamMenuPanel::CTeamMenuPanel(int iTrans, int iRemoveMe, int x,int y,int wide,int tall) : CMenuPanel(iTrans, iRemoveMe, x,y,wide,tall)
{
	...
	pLabel->setText( "%s", gHUD.m_TextMessage.BufferedLocaliseTextString("Select your team"));
	...
	for (int i = 1; i <= 5; i++)
	{
		...
		if (i == 5)
		{
			...
			m_pButtons[5]->setText(gHUD.m_TextMessage.BufferedLocaliseTextString("AutoAssign"));
