...
void IN_ResetMouse( void )
{
	...
	if ( gglobals && gglobals->time - s_flRawInputUpdateTime > 1.0f )
	{
		s_flRawInputUpdateTime = gglobals->time;
	...
	if (iMouseInUse || g_iVisibleMouse)
	{
		...
		for (i = 0; i<mouse_buttons; i++)
		{
			if ((mstate & (1 << i)) && !(mouse_oldbuttonstate & (1 << i)))
				gGui->mouseClick(i + 1, 1);
			if (!(mstate & (1 << i)) && (mouse_oldbuttonstate & (1 << i)))
				gGui->mouseClick(i + 1, 0);
		}
		return;
	}
...
void IN_MouseMove ( float frametime, usercmd_t *cmd)
{
	...
	if (!iMouseInUse && !gHUD.m_iIntermission && !g_iVisibleMouse)
	{
		...
		{
			gHUD.m_mouseSteps.x += mx_accum;
			gHUD.m_mouseSteps.y += my_accum*-1;

			float maxMouseRange = 255;

			if (gHUD.m_mouseSteps.y > maxMouseRange)
				gHUD.m_mouseSteps.y = maxMouseRange;
			...
			float old_value = gHUD.m_mouseSteps.y;
			float	old_min = -maxMouseRange;
			float	old_max = maxMouseRange;
		...
		//	gHUD.m_mouseSteps.z = new_value;
			gHUD.m_mouseSteps.z = gHUD.m_mouseSteps.y;
			if (gHUD.m_mouseSteps.z < 0)
				gHUD.m_mouseSteps.z = gHUD.m_mouseSteps.y*-1;

			
			gEngfuncs.Con_Printf("mx_accum:%i \n", int(mx_accum));
			if (mx_accum!=0)
	//		if (gEngfuncs.pfnRandomLong(0, 100)>80)
			PlaySound("items/lock.wav", 1);
		//	gEngfuncs.Con_Printf("test_valx:%i - test_valy:%i - test value %i\n", int(gHUD.m_mouseSteps.x), int(gHUD.m_mouseSteps.y), int(gHUD.m_mouseSteps.z));

			if (gHUD.m_mouseSteps.z < 2)
			{
			//	gEngfuncs.Con_Printf("cracked!\n");

				gHUD.lock_id++;

				if (gHUD.lock_id>2)
					gHUD.lock_id = 0;
				if (gEngfuncs.pfnRandomLong(0, 100)<50)
					gHUD.m_mouseSteps.y = gEngfuncs.pfnRandomLong(-255, -150);
				else
					gHUD.m_mouseSteps.y = gEngfuncs.pfnRandomLong(255, 150);
			}
		}
	...
	if (gHUD.m_Tbutton.m_iFlags & HUD_ACTIVE || in_strafe.state & 1)
	{
		//do nothing
	}
	else
