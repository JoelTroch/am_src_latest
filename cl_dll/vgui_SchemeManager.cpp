...
SchemeHandle_t CSchemeManager::LoadScheme(const char *schemeName)
{
	...
	{
		gEngfuncs.Con_Printf("ERROR: too many schemes!\n");
		return 0;
	}
	...
	if (!pFontData)
	{
		gEngfuncs.Con_Printf("Failed to load bitmap font for scheme \"%s\"\n", schemeName);
	...
	else
	{
		...
		{
			gEngfuncs.Con_Printf("Loading bitmap font \"%s\" using width info file\n", fontFilename);
		...
		{
			if (pWidthData && wdataFileLength != 768)
				gEngfuncs.Con_Printf("ERROR: Bogus width info file for font \"%s\"\n", fontFilename);
			else
				gEngfuncs.Con_Printf("Loading bitmap font \"%s\"\n", fontFilename);
