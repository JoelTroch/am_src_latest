...
extern int g_flashlight[MAX_PLAYERS+1];

void SetupFlashlight(cl_entity_t *player, float time, float frametime);
// p_render
...
void V_CalcViewRoll ( struct ref_params_s *pparams )
{
	...
	float rollie_angle = 1;
	float rollie_speed= 300;

 	pparams->viewangles[ROLL] = V_CalcRoll(pparams->viewangles, pparams->simvel, rollie_angle, rollie_speed) * 4;//SYS add
...
void V_CalcViewModelLag(ref_params_t *pparams, vec3_t &origin, vec3_t &angles, vec3_t original_angles)
{
	...
	if (pparams->frametime != 0.0f)	// not in paused
	{
		...
		if ((flDiff > gEngfuncs.pfnGetCvarFloat("cl_lazy_weapon")) && (gEngfuncs.pfnGetCvarFloat("cl_lazy_weapon") > 0.0f))
		{
			float flScale = flDiff / gEngfuncs.pfnGetCvarFloat("cl_lazy_weapon");
			flSpeed *= flScale;

		//	gEngfuncs.Con_Printf("added size %f\n", flDiff);
			gHUD.m_Health.crosssize += flDiff;
		}
		...
		origin = origin + (vDifference * -1.0f) * 3.0f;//5
	}
	...
	if (gEngfuncs.pfnGetCvarFloat("cl_lazy_weapon") <= 0.0f)
	{
...
void V_CalcNormalRefdef(struct ref_params_s *pparams)
{
	...
	SetupFlashlight(gEngfuncs.GetEntityByIndex(i), pparams->time, pparams->frametime); // p_render
	...
	Vector lastAngles = view->angles; //SYS ADD
...
/*	for (i = 0; i < 3; i++)
	{
		float delta = v_lastAngles[i] - pparams->viewangles[i];
		if (delta > 180)
			delta -= 360;
		if (delta < -180)
			delta += 360;
		...
		if (delta < -45)
			delta = -45;
		if (i == YAW)
			view->angles[ROLL] += 0.15f * delta;
		view->angles[i] += 0.3f * delta;
	}*/
	...
	if (gHUD.m_iFOV >= 87 )
	{
		...
		V_CalcViewModelLag(pparams, view->origin, view->angles, lastAngles);//SYS ADD

		if (speed > 0)
		speed -= (speed - 0) * pparams->frametime *3;

		if (speed < 0)
			speed = 0;

		if (speedZ > 0)
			speedZ -= (speedZ - 0) * pparams->frametime *3;
	...
	{
		if (gHUD.m_iFOV == 60)//rifles and etc
		{
			for (i = 0; i < 3; i++)
			{
				if (speed < CVAR_GET_FLOAT("origin_x"))
					speed -= (speed - CVAR_GET_FLOAT("origin_x")) * pparams->frametime * 5;

				if (speed > CVAR_GET_FLOAT("origin_x"))
					speed = CVAR_GET_FLOAT("origin_x");
			}
		}
		else//pistols?
		{
		...
		/*	for (i = 0; i < 3; i++)
			{
				view->origin[i] += g_vecRight[i] * -1;
				view->origin[i] += g_vecUp[i] * -1;
			}*/
		}

		if (speedZ < CVAR_GET_FLOAT("origin_z"))
			speedZ -= (speedZ - CVAR_GET_FLOAT("origin_z")) * pparams->frametime * 5;

		if (speedZ > CVAR_GET_FLOAT("origin_z"))
			speedZ = CVAR_GET_FLOAT("origin_z");
	}

	for (i = 0; i < 3; i++)
	{
		view->origin[i] += g_vecRight[i] * -speed;
		view->origin[i] += g_vecUp[i] * speedZ;
	}

//	gEngfuncs.Con_Printf("speed %f\n", speed);

 //	gEngfuncs.Con_Printf("m_iFOV %i\n", gHUD.m_iFOV);
	...
	//FIXME		I_Error ("steptime < 0");
		...
		if (pparams->simorg[2] - oldz > 28)
			oldz = pparams->simorg[2]- 28;
...
int V_FindViewModelByWeaponModel(int weaponindex)
{
	...
	static char * modelmap[][2] =	{
		{ "models/weapons/knife/p_knife.mdl",				"models/weapons/knife/v_knife.mdl"				},
		{ "models/weapons/flash/p_flash.mdl",				"models/weapons/flash/v_flash.mdl"				},
		{ "models/null.mdl",								"models/weapons/pda/v_pda.mdl"					},
		{ "models/weapons/fists/p_fists.mdl",				"models/weapons/fists/v_fists.mdl"				},
		{ "models/null.mdl",								"models/weapons/semtex/v_semtex.mdl"			},
		{ "models/null.mdl",								"models/weapons/semtex/v_semtex_radio.mdl"		},
		{ "models/weapons/deagle/p_deagle.mdl",				"models/weapons/deagle/v_deagle.mdl"			},
		{ "models/weapons/1911/p_1911.mdl",					"models/weapons/1911/v_1911.mdl"				},
		{ "models/weapons/ber92f/p_ber92f.mdl",				"models/weapons/ber92f/v_ber92f.mdl"			},
		{ "models/weapons/glock18/p_glock18.mdl",			"models/weapons/glock18/v_glock18.mdl"			},
		{ "models/weapons/357/p_357.mdl",					"models/weapons/357/v_357.mdl"					},
		{ "models/weapons/mp5/p_mp5.mdl",					"models/weapons/mp5/v_mp5.mdl"					},
		{ "models/weapons/p90/p_p90.mdl",					"models/weapons/p90/v_p90.mdl"					},
		{ "models/weapons/bizon/p_bizon.mdl",				"models/weapons/bizon/v_bizon.mdl"				},
		{ "models/weapons/famas/p_famas.mdl",				"models/weapons/famas/v_famas.mdl"				},
		{ "models/weapons/famas/p_famas.mdl",				"models/weapons/famas/v_famas_scope.mdl"		},
		{ "models/weapons/m4a1/p_m4a1.mdl",					"models/weapons/m4a1/v_m4a1.mdl"				},
		{ "models/weapons/ak47/p_ak47.mdl",					"models/weapons/ak47/v_ak47.mdl"				},
		{ "models/weapons/ak47/p_ak47.mdl",					"models/weapons/ak47/v_ak47_scope.mdl"			},
		{ "models/weapons/m249/p_m249.mdl",					"models/weapons/m249/v_m249.mdl"				},
		{ "models/weapons/spas12/p_spas12.mdl",				"models/weapons/spas12/v_spas12.mdl"			},
		{ "models/weapons/lesslethal/p_lesslethal.mdl",		"models/weapons/lesslethal/v_lesslethal.mdl"	},
		{ "models/weapons/usas/p_usas.mdl",					"models/weapons/usas/v_usas.mdl"				},
		{ "models/weapons/m14/p_m14.mdl",					"models/weapons/m14/v_m14.mdl"					},
		{ "models/weapons/m14/p_m14.mdl",					"models/weapons/m14/v_m14_scope.mdl"			},
		{ "models/weapons/l96a1/p_l96a1.mdl",				"models/weapons/l96a1/v_l96a1.mdl"				},
		{ "models/weapons/l96a1/p_l96a1.mdl",				"models/weapons/l96a1/v_l96a1_scope.mdl"		},
		{ "models/weapons/rpg/p_rpg.mdl",					"models/weapons/rpg/v_rpg.mdl"					},
		{ NULL, NULL } };
...
void CL_DLLEXPORT V_CalcRefdef( struct ref_params_s *pparams )
{
	...
	gl.m_vViewAngles = render_angles;
	gl.m_vRenderOrigin = render_origin;

	AngleVectors(render_angles, g_vecForward, g_vecRight, g_vecUp); // for XaeroX's sprite rendering..
...
void SetupFlashlight(cl_entity_t *player, float time, float frametime)
{
	...
	vec3_t angles = player->curstate.angles;
	...
	DynamicLight *flashlight = MY_AllocDlight(-666 - idx);
	//flashlight->origin = origin + Vector(0,0,5) + (right*5);
	flashlight->origin = origin + Vector(0,0,28) + (right*5);
