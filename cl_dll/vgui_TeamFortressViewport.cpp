...
#include "vgui_OS.h"//COS
#include "vgui_weapon_panel.h"//CWeaponMenuPanel

#include "vgui_pickup.h"
...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	m_pShepardEasterEgg = new CShepardEasterEgg();
}
...
void TeamFortressViewport::Initialize( void )
{
	...
	if (m_pWeaponMenu)
	{
		m_pWeaponMenu->setVisible(false);
		UpdateCursorState();
	}
...
void TeamFortressViewport::UpdateCursorState()
{
	...
	if (m_pSpectatorPanel->m_menuVisible || m_pCurrentMenu || m_pServerBrowser->isVisible() || GetClientVoiceMgr()->IsInSquelchMode() || m_pWeaponMenu && m_pWeaponMenu->isVisible() || m_pOS && m_pOS->isVisible())
...
int	TeamFortressViewport::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	if (m_pSubtitle && m_pSubtitle->isVisible())
		return m_pSubtitle->KeyInput(down, keynum, pszCurrentBinding);

	// buz
	if (m_pWeaponMenu && m_pWeaponMenu->isVisible())
		return m_pWeaponMenu->KeyInput(down, keynum, pszCurrentBinding);

	if (m_pOS && m_pOS->isVisible())
		return m_pOS->KeyInput(down, keynum, pszCurrentBinding);
	...
	if ( down && m_pShepardEasterEgg && gHUD.m_iAllowShepardEasterEgg == 1 )
	{
		m_pShepardEasterEgg->KeyInput( down, keynum, pszCurrentBinding );
		return 0;
	}
...
int TeamFortressViewport::MsgFunc_Recoil(const char *pszName, int iSize, void *pbuf)
{
	vec3_t viewangles;

	BEGIN_READ(pbuf, iSize);

	float fRecoil = READ_BYTE();

//	gEngfuncs.GetViewAngles((float *)viewangles);
	
	V_PunchAxis(0, -fRecoil);
	//viewangles[PITCH] -= READ_BYTE();
	
//	gEngfuncs.SetViewAngles((float *)viewangles);
	gHUD.m_Health.crosssize += fRecoil;
...
int TeamFortressViewport::MsgFunc_ShowWeaponMenuWindow(const char *pszName, int iSize, void *pbuf)
{
	BEGIN_READ(pbuf, iSize);

	m_pWeaponMenu = new CWeaponMenuPanel();
	m_pWeaponMenu->setParent(this);
	m_pWeaponMenu->iNumButtons = 0;

	if (m_pWeaponMenu)
	{
		m_pWeaponMenu->sWeaponModifiers.sScope.iValue = READ_BYTE();
		m_pWeaponMenu->sWeaponModifiers.sSilencer.iValue = READ_BYTE();
		m_pWeaponMenu->sWeaponModifiers.sRedDot.iValue = READ_BYTE();
		m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iValue = READ_BYTE();
		m_pWeaponMenu->sWeaponModifiers.sGL.iValue = READ_BYTE();
		m_pWeaponMenu->sWeaponModifiers.sLaser.iValue = READ_BYTE();
		m_pWeaponMenu->sWeaponModifiers.sFireMode.iValue = READ_BYTE();

		//set order - not necessary, I guess
		m_pWeaponMenu->sWeaponModifiers.sScope.iOrder = -1;
		m_pWeaponMenu->sWeaponModifiers.sSilencer.iOrder = -1;
		m_pWeaponMenu->sWeaponModifiers.sRedDot.iOrder = -1;
		m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iOrder = -1;
		m_pWeaponMenu->sWeaponModifiers.sGL.iOrder = -1;
		m_pWeaponMenu->sWeaponModifiers.sLaser.iOrder = -1;
		m_pWeaponMenu->sWeaponModifiers.sFireMode.iOrder = -1;

	//	CONPRINT("iNumButtons %i = order before : %i-%i-%i-%i-%i-%i\n", m_pWeaponMenu->iNumButtons, m_pWeaponMenu->sWeaponModifiers.sScope.iOrder, m_pWeaponMenu->sWeaponModifiers.sSilencer.iOrder, m_pWeaponMenu->sWeaponModifiers.sRedDot.iOrder, m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iOrder, m_pWeaponMenu->sWeaponModifiers.sGL.iOrder, m_pWeaponMenu->sWeaponModifiers.sLaser.iOrder);

		//quick and nasty way to determine how many of these are on
		if (m_pWeaponMenu->sWeaponModifiers.sFireMode.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sFireMode.iOrder = m_pWeaponMenu->iNumButtons;
		}

		if (m_pWeaponMenu->sWeaponModifiers.sScope.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sScope.iOrder = m_pWeaponMenu->iNumButtons;
		}

		if (m_pWeaponMenu->sWeaponModifiers.sSilencer.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sSilencer.iOrder = m_pWeaponMenu->iNumButtons;
		}

		if (m_pWeaponMenu->sWeaponModifiers.sRedDot.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sRedDot.iOrder = m_pWeaponMenu->iNumButtons;
		}

		if (m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iOrder = m_pWeaponMenu->iNumButtons;
		}

		if (m_pWeaponMenu->sWeaponModifiers.sGL.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sGL.iOrder = m_pWeaponMenu->iNumButtons;
		}

		if (m_pWeaponMenu->sWeaponModifiers.sLaser.iValue != MOD_NOT_AVAILABLE)
		{
			m_pWeaponMenu->iNumButtons++;
			m_pWeaponMenu->sWeaponModifiers.sLaser.iOrder = m_pWeaponMenu->iNumButtons;
		}


	//	CONPRINT("sScope: %i  sSilencer: %i sRedDot: %i  sExtendedMag: %i sGL: %i sLaser: %i\n", m_pWeaponMenu->sWeaponModifiers.sScope.iOrder, m_pWeaponMenu->sWeaponModifiers.sSilencer.iOrder, m_pWeaponMenu->sWeaponModifiers.sRedDot.iOrder, m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iOrder, m_pWeaponMenu->sWeaponModifiers.sGL.iOrder, m_pWeaponMenu->sWeaponModifiers.sLaser.iOrder);

	//	CONPRINT("buttons info: %i-%i-%i-%i-%i-%i\n", m_pWeaponMenu->sWeaponModifiers.sScope.iValue, m_pWeaponMenu->sWeaponModifiers.sSilencer.iValue, m_pWeaponMenu->sWeaponModifiers.sRedDot.iValue, m_pWeaponMenu->sWeaponModifiers.sExtendedMag.iValue, m_pWeaponMenu->sWeaponModifiers.sGL.iValue, m_pWeaponMenu->sWeaponModifiers.sLaser.iValue);
	}

	m_pWeaponMenu->Init();
	m_pWeaponMenu->setVisible(true);
	UpdateCursorState();
	...
	if ( g_iUser1 == OBS_IN_EYE )
	{
		...
		if ( g_iUser2 == iIndex )
		{
			...
			if ( iFade == BUILD_TELEPORTER_FADE_OUT )
			{
				...
				sf.fadeTotalEnd = sf.fadeEnd += gglobals->time;
...
void TeamFortressViewport::AddPickupMessage(client_textmessage_t *msg)
{
	...
	if (msgpanel)
	{
		...
		gEngfuncs.Con_Printf("ACHTUNG! pickup msgpanel is not created!\n");
}
