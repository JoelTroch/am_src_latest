...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
		...
		pCurrent->m_fireState			= pfrom->iuser3; // Shepard : Was used by HL's Egon, can be either removed or used for something else
	...
	player.ammo_50			= (int)from->client.vuser1[1];
	player.ammo_45			= (int)from->client.ammo_nails; //is an int anyways...
	player.ammo_buckshot	= (int)from->client.ammo_shells; 
	player.ammo_44			= (int)from->client.ammo_cells;
	player.ammo_57			= (int)from->client.vuser1[2];
	player.ammo_rockets		= (int)from->client.ammo_rockets;
	player.ammo_556			= (int)from->client.vuser2[0];
	...
	to->client.vuser1[1]				= player.ammo_50;

	to->client.ammo_nails				= player.ammo_45;
	to->client.ammo_shells				= player.ammo_buckshot;
	to->client.ammo_cells				= player.ammo_44;
	to->client.vuser1[2]				= player.ammo_57;
	to->client.vuser2[0]				= player.ammo_556;
...
//		pto->m_flPumpTime				= pCurrent->m_flPumpTime;
		...
		pto->iuser3						= pCurrent->m_fireState; // Shepard : Was used by HL's Egon, can be either removed or used for something else
