...
void ProcessRain( void )
{
	...
	//gEngfuncs.Con_Printf("proccess rain\n");
	...
	while (rain_nextspawntime < rain_curtime)
	{
		...
		if (dripcounter < MAXDRIPS) // check for overflow
		{
			...
			gEngfuncs.pEventAPI->EV_PlayerTrace( vecStart, vecStart + vecEnd, PM_STUDIO_BOX, -1, &pmtrace );
			...
			int extraContents = gEngfuncs.PM_PointContents(pmtrace.endpos - pmtrace.plane.normal , NULL);

			if(extraContents != CONTENTS_SOLID && extraContents != CONTENTS_EMPTY
				&& extraContents != CONTENTS_SKY && extraContents != CONTENTS_WATER)
			...
			if (contents == CONTENTS_WATER || extraContents == CONTENTS_LAVA)
