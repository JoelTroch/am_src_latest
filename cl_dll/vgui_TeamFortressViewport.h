...
class CWeaponsSelPanel;
class CHud2;
class CWeaponMenuPanel;
...
public:
	int MsgFunc_ShowWeaponMenuWindow(const char *pszName, int iSize, void *pbuf);
	...
	int MsgFunc_Recoil(const char *pszName, int iSize, void *pbuf);
...
public:
	...
	CWeaponMenuPanel* m_pWeaponMenu;
	CHud2* m_pHud2;
	COpSys* m_pOS;
	...
	CShepardEasterEgg *m_pShepardEasterEgg;
...
class CWeaponsSelPanel : public Panel
{

private:

	CImageLabel	*m_pSlotNumberLabel[MAX_WEAPON_SLOTS];
	CImageLabel	*m_pSlotPreWpLabel[MAX_WEAPON_SLOTS];
	CImageLabel	*m_pSlotWeaponLabel[MAX_WEAPON_SLOTS][MAX_WEAPON_SLOTS];
	CImageLabel	*m_pSlotSelWeaponLabel[MAX_WEAPON_SLOTS][MAX_WEAPON_SLOTS];

public:

	CWeaponsSelPanel(int x, int y, int wide, int tall);
	virtual ~CWeaponsSelPanel(void);
