...
public:
	...
	void CreateCluster( char *szPath, vec3_t origin, vec3_t dir, int iId,vec3_t forceColor = Vector(0,0,0) );
	particle_system_t *CreateSystem( char *szPath, vec3_t origin, vec3_t dir, int iId, particle_system_t *parent = NULL,vec3_t forceColor = Vector(0,0,0) );
	...
	void DrawSpecialParticles(void);
	void CullSystems( void );
	void UpdateSystems( void );

	vec3_t LightForParticle( cl_particle_t *pParticle );
	bool CheckLightBBox(cl_particle_t *pParticle, DynamicLight *pLight);
