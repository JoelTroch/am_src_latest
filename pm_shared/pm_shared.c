...
void PM_PlayStepSound( int step, float fvol )
{
	...
	case 0:
	case 1:
		pmove->vuser3[0] = Length(pmove->velocity) / 18;//8
		pmove->vuser3[1] = Length(pmove->velocity) / 18;
		break;

		// left foot
	case 2:
	case 3:
		pmove->vuser3[0] = Length(pmove->velocity) / 18;
		pmove->vuser3[1] = -Length(pmove->velocity) / 18;
		break;
	}
...
void PM_WalkMove ()
{
	...
	// Shepard : Are we sprinting ?
	qboolean sprinting;
	if ( pmove->cmd.buttons & IN_SPRINT )
	{
		pmove->oldbuttons |= IN_SPRINT;
		sprinting = true;
	}
	else
	{
	...
	{
		if ( sprinting && !(pmove->flags & FL_DUCKING) )
...
void PM_UnDuck( void )
{
	...
	if (pmove->onground != -1 && pmove->flags & FL_DUCKING && pmove->bInDuck == false)//SYS ADD
	{
		for ( i = 0; i < 3; i++ )
		{
	...
	if ( trace.fraction != 1.0 )
	{
		...
		else
		{
			if ( forward != 0 || right != 0 )
			{
				...
				//Vector velocity = (forward * gglobals->v_forward) + (right * gglobals->v_right);
...
void PM_Jump (void)
{
	...
	switch (pmove->RandomLong(0, 3))
	{
		case 0: pmove->PM_PlaySound(CHAN_BODY, "player/jump01.wav", 0.9, ATTN_NORM, 0, PITCH_NORM);
			break;

		case 1: pmove->PM_PlaySound(CHAN_BODY, "player/jump02.wav", 0.9, ATTN_NORM, 0, PITCH_NORM);
			break;

		case 2: pmove->PM_PlaySound(CHAN_BODY, "player/jump03.wav", 0.9, ATTN_NORM, 0, PITCH_NORM);
			break;

		case 3: pmove->PM_PlaySound(CHAN_BODY, "player/jump04.wav", 0.9, ATTN_NORM, 0, PITCH_NORM);
			break;
	}
	//SYS ADD
...
void PM_CheckFalling( void )
{
	...
	{
		//SYS ADD
		float punch = pmove->flFallVelocity * 0.01;
		// pmove->Con_DPrintf("fall vel: %f, punch: %f\n", pmove->flFallVelocity, punch/20);
		pmove->vuser3[0] += punch;
		pmove->vuser3[1] += pmove->RandomFloat(punch / -10, punch / 10);
		pmove->vuser3[2] += pmove->RandomFloat(punch / -10, punch / 10);
...
void PM_DropPunchAngle(vec3_t punchangle)
{
		if ( pmove->multiplayer )
	{
		...
		{
			// buz: ó íàñ òåïåðü íîâûé, ñâåæåñïåðòûé èç õë2, ïóí÷.
			float damping;
